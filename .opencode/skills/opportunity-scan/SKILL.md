...
> **Project adaptation (ModernWigiDash/OpenCode):** this agent's session logs live
> in agentmemory (MCP: `agentmemory_memory_sessions`, `agentmemory_memory_recall`,
> `agentmemory_memory_smart_search`) — use those as the WINDOW OF LOGS target,
> aggregating via recall/smart-search rather than raw files. Capability docs to
> read in step 1: opencode.ai docs plus this repo's `.opencode/AGENTS.md`
> (extension points: rules, skills, subagents, MCP tools, hooks). Reports land in
> the repo `docs/` (e.g. `docs/reports/`), self-contained HTML, gitignored only
> if the user says so.
