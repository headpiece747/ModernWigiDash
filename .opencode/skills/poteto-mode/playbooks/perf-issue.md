...
1. Capture a baseline trace on the matching surface. UI-driven slowness: the `verify-modernwigidash` harness with timing around the driven feature. CPU/heap/runtime: GliderTrace (`trace_start` with `collectTrace`, `trace_attach`, counters via `collectCounters`). The 30 FPS render tick and the frame pipeline are the first suspects on this app.
2. `how` to ground hypotheses; don't claim a perf ceiling without running it first.
    ...
    - **Caching.** The same computation or fetch repeats on identical inputs. Store and reuse the result; name what invalidates it before claiming the win. Per-frame allocation in the render tick is the house-shaped instance.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent (session model); review the diff. Capture a post-fix trace.
    Apply the **sequence-verifiable-units** principle skill, verifying each attempt before trying the next.
4. Parse and compare the artifacts (nettrace hotspots via `trace_hotspots`, counters via `trace_counters`, before/after timings). "Inconclusive" or wrong-surface is not a pass; flag it.
5. Cite the measurement in the commit message and any run notes.
6. Close with the house gate: full release build plus the affected tests green.
