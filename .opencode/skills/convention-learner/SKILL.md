...
> glider_get_symbol_info; `get_project_graph` → glider_get_project_graph;
> `detect_antipatterns` → glider_semantic_query; `find_dead_code` →
> glider_find_unused_symbols. Convention docs to write/update: CONTEXT.md (domain
> glossary) and AGENTS.md.
