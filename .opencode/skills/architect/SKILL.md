...
Naming a file isn't grounding. Produce the traced model `how` prescribes. If the design redefines ownership or layering, ground the existing shape first: `docs/adr/`, `CONTEXT.md`, and `git log`/`git blame` so the rationale becomes a constraint, not a guess.
...
Run 2-3 fresh subagents as candidates (session model; the value is N independent structural attempts plus the cross-judge, not model diversity).
...
The synthesis can ship as its own commit either way. That's the "scaffold first" mode of the **foundational-thinking** principle skill; subsequent commits read as filling in bodies against a stable contract. Planned and scoped breakage during fill-in is fine, per the **outcome-oriented-execution** principle skill. For adversarial pressure on the design before implementing, run `ocr_review` plus the `code-reviewer` subagent on the synthesized sketch.
...
- Types that need escape hatches (`any`/dynamic, casts, always-nullable fields that are set in practice) to compile or pass review.
- The "we need a lock" reflex when the sketch said the state wasn't shared (this repo runs single-threaded UI + a few named poll loops; a new lock is a sketch smell first, a concurrency problem second).
...
The caller's usage is written first and the type sketch derived from it. One file with new types and signatures for small changes; module map plus type definitions for larger work. The rationale ships alongside, shaped per `references/rationale-template.md`, including the usage sketch and the synthesis decision. Commit the sketch (with `not implemented` bodies) before filling in, so the contract has its own history.