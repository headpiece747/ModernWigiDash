...
- **Open.** Run `click-id BtnSettings`. Then `wait "Settings" -Seconds 10` returns the hub window. Proof: `dump -Path <evidence>/settings/hub.txt` shows a window titled `Settings` whose text includes the group headers `APPEARANCE`, `BEHAVIOR`, and `PROFILE`.
- **Persist the close behavior.** In the Behavior group, run `click "Keep running in the tray when the window closes"`. Proof (second view): the persisted `profile.json` (the app's profile file in the user state dir, `%LOCALAPPDATA%\ModernWigiDash\profile.json`, read directly) now carries `"CloseBehavior": "hideToTray"`. The write is immediate: no Apply, no close step between the check and the read.
- **Restore the default.** Run `click "Quit when the window closes"`. Proof: `profile.json` carries `"CloseBehavior": "quit"`.
- **Hide on close.** With the keep-alive setting persisted (the step above, reversed), close the main window: read the window's close box (the `Close` button in the `dump` of the main window) and `click-at <x> <y>` it (needs the synthetic mouse; an agent session without one is precondition-blocked, use the `stop` alternative below). Proof: `wait "ModernWigiDash"` times out (the window is hidden, not closed) and the app process is still alive (the harness `doctor` process read still lists it). Minimize behaves the same: with the setting on, a minimize also hides to the tray. Agent-session alternative: `stop` drives the app's own close path (WM_CLOSE, the same WPF `Closing` event the X button raises) and verdicts `hid instead of exiting` (the close intercept cancels the close; the process stays alive and the window is gone).
- **Restore the hidden window.** Launch the app the way a user does (double-click the exe or the Start-menu shortcut): `Start-Process` the launched exe path directly (the harness `launch` refuses while its recorded instance is alive, the shared-instance rule). Proof: `wait "ModernWigiDash" -Seconds 15` returns the main window (the first instance, the same pid) and the second process has exited, the single-instance guard's activation brought the hidden window forward. (The tray icon's own single-click show is user-manual: the icon lives in the OS notification area, outside the app's UIA tree.)
...
- **Export/Import buttons.** The Profile group's Export and Import buttons carry AutomationIds `BtnExportProfile` and `BtnImportProfile`. Use `click-id BtnExportProfile` / `click-id BtnImportProfile` to open the Save/Open dialogs (a bare `click "Export profile..."` is shadowed by the description text containing "exported"). See [Profile round trip](./profile-roundtrip.md) for the full recipe.
