...
- The radios persist the setting. The runtime close intercept (hiding instead of exiting) is the tray/close-behavior surface and lands in the same feature set; until then, closing with the setting on still exits the app.