...
- The `Connected` label text is amber (`M3Primary`) by design: green is the DOT's job. A stale theme (the state-dir `app_theme.json`, the theme's real location since ADR-0021, or a legacy exe-dir copy that migrates once when the state file is absent) can re-color `AccentGreen` to a near-background value and make the dot invisible while the label still reads `Connected` in amber: check the state-dir theme file first and the exe-dir legacy copy too, before calling the badge wrong (see `theme.md`).
