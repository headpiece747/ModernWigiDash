...
- `export` writes the profile (plus the optional `"theme"` section, ADR-0021) to a user-chosen path via the Save dialog.
- `import` loads a profile file via the Open dialog and rehydrates the pages/widgets; a bundle carrying a different theme is offered behind one `Restore Theme` confirm.
...
- **Export.** Open the hub (`click-id BtnSettings`). Run `click-id BtnExportProfile` in the hub's Profile group (a bare `click "Export profile..."` is shadowed by the description text containing "exported"; the id avoids this). A Save dialog (common file dialog, window title `Save As` or the OS localization) appears. Set the file name to the scratch path and confirm: the Save dialog's file-name Edit is usually the Edit control inside the dialog, locate it with `dump -Path <evidence>/profile/savedialog.txt` (look for a dialog whose children include a button named `Save`), then `set <file-name-edit> <scratch-path>.json` and `click Save`. Proof (second view): the file exists at the scratch path and parses: it is JSON, has a `pages` array, and that array's element count matches the page tabs (`find "📄 "` count); it also carries the top-level `theme` object (the bundle's theme section, ADR-0021, matching the current state-dir `app_theme.json`).
- **Import.** With the hub open, run `click-id BtnImportProfile`. The Open dialog appears (same handle story: find the `Open` button). `set <file-name-edit> <scratch-path>.json`, `click Open`. The app rehydrates: the tab strip matches the imported page set and `value ActiveCount` matches the imported widget count. Proof: `find "📄 "` now lists the imported page names; `value ActiveCount` equals the JSON's element count; `shot <evidence>/profile/after-import.png`.
- **Oversize gate.** Create a scratch file larger than the app's import cap (the sanitizer's `MaxImportFileBytes`; check the constant before choosing, do not hardcode a guessed size) with a `profile.json`-shaped but oversized body. Import it (`click-id BtnImportProfile`, choose the scratch file); the app refuses with an error dialog (`find "OK"` in a message window). Proof (negative): the dialog text names the size problem, the profile is unchanged (`find "📄 "` and `value ActiveCount` as before), and `restore-profile` is then run via `clean`.
...
- The `Restore Theme` confirm (ADR-0021) appears only when the imported bundle carries a theme whose fingerprint differs from the current one (a default-themed export never prompts a default-themed machine). It is offered AFTER the profile swap: declining it leaves `app_theme.json` in the state dir untouched and never undoes the imported profile; accepting applies the colors to the session and persists them. A bundle whose `"theme"` section is absent, null-valued, or malformed imports normally with the theme untouched (the read never throws).
- Deleting `%LOCALAPPDATA%\ModernWigiDash\profile.json` is NOT a visible reset when the user's dashboard is the starter dashboard: the next launch finds no file, rebuilds the 6-page `StarterProfile`, and re-persists it ~3 s later (the 2 s save debounce). The fresh file's GUIDs match no backup (a trusted load skips the sanitizer, so page ids survive and the lineage is dateable against the `profile.json.*-backup` files in the state dir). "The old profile came back" after a delete is a misread: it is a fresh build of the same default layout (verified 2026-08-26: identical layout and property values, only new GUIDs).
