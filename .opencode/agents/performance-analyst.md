...
> **Tool mapping (this project):** `find_references` → glider_find_references,
> `find_symbol` → glider_search_symbols, `get_public_api` →
> glider_get_type_info, `get_diagnostics` → glider_get_diagnostics. Hot-path
> evidence can also come from glider-trace (runtime.nettrace CPU hotspots).
