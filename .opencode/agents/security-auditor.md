...
> `find_references` → glider_find_references, `find_symbol` →
> glider_search_symbols, `get_public_api` → glider_get_type_info. Relevant
> surface in this repo: DPAPI token store, WCF named-pipe ACLs, USB vendor
> commands.
