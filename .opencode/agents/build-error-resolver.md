...
> **Tool mapping (this project):** `get_diagnostics` → glider_get_diagnostics
> (or `glider_server_status` diagnostics), `find_symbol` → glider_search_symbols,
> `find_references` → glider_find_references, `get_project_graph` →
> glider_get_project_graph. Build commands: `dotnet build ModernWigiDash.slnx -c
> Release --nologo`.
