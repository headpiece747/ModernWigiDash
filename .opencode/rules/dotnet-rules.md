...
- Every `[DllImport]` spells its `EntryPoint` explicitly (the `DebtGuardTests` pin): without it, the binding resolves to the method name, so a rename silently re-targets the call and a never-existing export throws `EntryPointNotFoundException` on the first real call (the 2026-08-26 hotkey crash). `PInvokeBindingTests` probes each spelled (dll, entry point) against the real DLL at the gate (ADR-0020); a new extern is covered automatically by the sweep.
...
- **Every prose surface runs the unslop pass.** Docs, ADRs, CONTEXT.md, commit messages, and agent replies are written per the `unslop` skill (plain words, concrete facts, no puffery, no AI vocabulary, straight quotes). Em dashes are discouraged: prefer a period, a comma, or a new sentence, and a single well-earned em dash is allowed when it is the clearest separator (the 2026-08-27 decision relaxed the ban and retired the gate's prose scan; arrows (→), word-internal hyphens, and en dashes in numeric ranges are not dashes and stay). Quoted example strings that cite a pattern (like the ADR-0009 hint string) are exempt. The scope is prose: `.cs` comments and the dated records under `docs/archive`, `docs/superpowers`, and `docs/reports` are out of scope by 2026-08-23 decision. Do not run the pass over them.
- **Debt is prevented by structure, not by review.** The mechanical anti-patterns (sync-over-async outside the budgeted sites, `async void` outside event handlers, unowned handles, frame-pipeline encode/pool bypass, dead private helpers, unspelled P/Invoke entry points) are machine-pinned in `DebtGuardTests` and run in the gate's test stage; the gate guard blocks a commit without a green gate, so these rules execute before every commit instead of waiting for a review finding. The code-reviewer agent covers the judgment layer the pins cannot see: is an allow-list reason true, is a new abstraction earning its place, does a Dispose path release every handle on every failure leg.
