...
public class WindowNavigatePageTests
{
    ...
    public void WmHotkey_RoutesThroughTheHook_ToTheOwningWidgetFirePath()
    {
        ...
        Host.Run<object?>(() =>
        {
            var window = new MainWindow(new MainWindowTestOptions(new StubPresentMonNative(), profilePath, new NoopPowerModeSource(), new FakeTraySurface(), HotkeyApi: fake.Api, UsbEngine: FakeTransport.InertEngine()));
            try
            {
                window.Show();
