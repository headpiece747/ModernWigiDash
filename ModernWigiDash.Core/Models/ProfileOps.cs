...
/// <summary>
/// Profile operations: page CRUD, widget placement (the rehydration sequencing
/// lives in <see cref="WidgetRehydrator"/>), and JSON export/import. The window
/// keeps only dialogs, selection, and refresh. Widgets are rehydrated through
/// the loader + host context, so a profile round-trips Export→Import with its
/// custom property values intact.
/// </summary>
public static class ProfileOps
{
    ...
    /// rehydrates the instance (the create/init/property/dispose sequencing
    /// lives in <see cref="WidgetRehydrator"/>), sizes it (explicit or the
    /// widget's default), and assigns the next ZIndex. Returns null when the
    /// plugin is unknown.
    /// </summary>
        ...
        float height = -1)
    {
        ...
        var instance = WidgetRehydrator.Rehydrate(loader, context, placed);
    ...
    /// cannot abort the whole import. The sequencing itself lives in
    /// <see cref="WidgetRehydrator"/>; this entry keeps the historical name
    /// for callers that reference it directly.
    /// </summary>
    public static IModernWidget? RehydrateWidget(
        WidgetPluginLoader loader,
        IModernWigiDashContext context,
        PlacedWidgetInstance placed)
        => WidgetRehydrator.Rehydrate(loader, context, placed);

    /// <summary>
    /// Tears down a placed widget's active instance (one spelling, shared by
    /// every page-mutation path; the rules live in
    /// <see cref="WidgetRehydrator.DisposeInstance"/>).
    /// </summary>
